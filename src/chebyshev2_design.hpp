...
namespace FilterDesigner {
    ...
    template<typename precision, size_t order>
    class Chebyshev2Bandstop : public IIRGenericDesign<precision, order * 2, order * 2, order> {
    public:
        /* Chebyshev Type 2 band-stop filter constructor
         * @param sample_rate [Hz] Sampling rate of the input signal
         * @param first_cut   [Hz] Lower cutoff frequency for the band-stop filter
         * @param second_cut  [Hz] Upper cutoff frequency for the band-stop filter
         * @param stopband_atten_db [dB] Stopband attenuation in decibels (typically 20 to 80 dB) */
        Chebyshev2Bandstop(const precision& sample_rate, const precision& first_cut, const precision& second_cut, const precision& stopband_atten_db = precision(40)) : m_stopband_atten_db(stopband_atten_db) {
            //Validates stopband attenuation parameter
            if (stopband_atten_db <= 0) m_stopband_atten_db = precision(40);
    ...
    protected:
        ...
        auto convertPrototype() -> bool override {
            ...
            //Transform low-pass prototype to band-stop
            //Each pole/zero becomes two poles/zeros
            for (size_t index = 0; index < num_analogue_poles; index++) {
                const precision pole_magnitude = cabs(this->m_poles[index]);
                const precision zero_magnitude = cabs(this->m_zeros[index]);
                ...
                //Transform pole: LP to BS (radial inversion followed by a frequency shift)
                //If it's a real pole
                if (abs(this->m_poles[index].imag()) < precision(1e-12) && index == num_analogue_poles - 1 && (order % 2 == 1)) {

                    const Complex<precision> base = Complex<precision>(bandwidth) / this->m_poles[index];
                    const Complex<precision> omega_j = Complex<precision>(0, omega);

                    temp_poles[index] = precision(0.5) * base + omega_j;
                    temp_poles[index + num_analogue_poles] = precision(0.5) * base - omega_j;
                ...
                //Complex pole
                    ...
                    const Complex<precision> base = Complex<precision>(bandwidth) / this->m_poles[index];
                    const Complex<precision> discriminant = base * base - Complex<precision>(4) * omega * omega;

                    const Complex<precision> first  = precision(0.5) * base;
                    const Complex<precision> second = precision(0.5) * csqrt(discriminant);
                ...
                //Transform zero: LP to BS
                if (zero_magnitude > precision(1e-12) && zero_magnitude < precision(1e9)) {
                    //If it's a real zero (shouldn't occur for Type 2 imaginary zeros)
                    if (abs(this->m_zeros[index].imag()) < precision(1e-12) && index == num_analogue_poles - 1 && (order % 2 == 1)) {

                        const Complex<precision> base = Complex<precision>(bandwidth) / this->m_zeros[index];
                        const Complex<precision> omega_j = Complex<precision>(0, omega);

                        temp_zeros[index] = precision(0.5) * base + omega_j;
                        temp_zeros[index + num_analogue_poles] = precision(0.5) * base - omega_j;
                    }
                    ...
                    //Zero at infinity becomes zeros at the center of the stopband
                    temp_zeros[index] = Complex<precision>(0, omega);
                    temp_zeros[index + num_analogue_poles] = Complex<precision>(0, -omega);
                }
            }
            ...
            //Calculate initial gain (unity gain outside the stopband)
            precision initial_gain = precision(1);

            //Performs the plane conversion
            this->m_overall_gain = planeConversion(initial_gain, this->m_poles, this->m_num_poles, this->m_zeros, this->m_num_zeros);

            //Normalize to ensure proper gain outside the stopband
            this->m_overall_gain = precision(1) / this->m_overall_gain;
