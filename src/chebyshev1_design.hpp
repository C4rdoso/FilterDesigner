...
namespace FilterDesigner {
    ...
    template<typename precision, size_t order>
    class Chebyshev1Bandstop : public IIRGenericDesign<precision, order * 2, order * 2, order> {
    public:
        /* Chebyshev Type 1 band-stop filter constructor
         * @param sample_rate [Hz] Sampling rate of the input signal
         ...
         * @param ripple_db   [dB] Passband ripple in decibels (typically 0.1 to 3 dB) */
        Chebyshev1Bandstop(const precision& sample_rate, const precision& first_cut, const precision& second_cut,const precision& ripple_db = precision(0.5)) : m_ripple_db(ripple_db) {
            //Validates ripple parameter, if not valid, assigns a default value
            if (ripple_db <= 0) m_ripple_db = precision(0.5);
        ...
        auto convertPrototype() -> bool override {
            ...
            for (size_t index = 0; index < num_analogue_poles; index++) {
                ...
                if (abs(this->m_poles[index].imag()) < precision(1e-12) &&
                    ...
                    Complex<precision> first_half  = precision(0.5) * base + omega_j;
                    Complex<precision> second_half = precision(0.5) * base - omega_j;
                ...
                //Places the transformed zero pair at the notch frequency, on the imaginary axis
